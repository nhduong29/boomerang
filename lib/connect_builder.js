...
ConnectBuilder.prototype.about = function about(path, files, filesNamespace, themeOpt) {
  const theme = themeOpt || 'default';
  var that = this;
  this.app.use('/about',function(req, res){
...
ConnectBuilder.prototype.api = function api(path, files, filesNamespace, themeOpt) {
  const theme = themeOpt || 'default';
  var that = this;
...
ConnectBuilder.prototype.contact = function contact(path, files, filesNamespace, themeOpt) {
  const theme = themeOpt || 'default';
  var that = this;
  this.app.use('/contact',function(req, res){
    fs.readFile(path, (err, data) => {
      res.writeHead(200, {
        'Content-Type': 'text/html',
      });
...
ConnectBuilder.prototype.advance = function advance(path, files, filesNamespace, themeOpt) {
  const theme = themeOpt || 'default';
  var that = this;
  this.app.use('/advance',function(req, res){
    fs.readFile(path, (err, data) => {
      res.writeHead(200, {
        'Content-Type': 'text/html',
      });
